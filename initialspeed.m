...
options = optimoptions('ga', ...
   ...
   % 'HybridFcn', @patternsearch); % Use pattern search for hybrid function
...
for u = 1:U
    % Debugging print statement for initial speeds
    %disp(['Initial UAV ', num2str(u), ' speeds: ', num2str(x0.S_x(u, :))]);
    plot(1:N, x0.S_x(u, :), '--o', 'DisplayName', ['Initial UAV ' num2str(u)]);
end

% Plot optimized UAV speeds
for u = 1:U
    % Debugging print statement for optimized speeds
    %disp(['Optimized UAV ', num2str(u), ' speeds: ', num2str(sol.S_x(u, :))]);
    plot(1:N, sol.S_x(u, :), '-x', 'DisplayName', ['Optimized UAV ' num2str(u)]);
end

xlabel('Time Slot','FontSize',25);
ylabel('Speed','FontSize',25);
title('Initial and Optimized UAV Speeds Over Time','FontSize',25);
legend('show');
grid on;
hold off;
...
figure;
plot(1:N, total_energy_before/1e2, '-o', 'DisplayName', 'Energy Consumption Pre Optimization');
hold on;
plot(1:N, total_energy_after/1e2, '-x', 'DisplayName', 'Energy Consumption Post Optimization');
xlabel('Time Slot','FontSize',25);
ylabel('Total Energy Consumption (W)','FontSize',25);
legend('show');
title('Total Energy Consumption Before and After Optimization','FontSize',25);
hold off;
% Calculate distances between UAV and vehicles before and after optimization
distances_before = zeros(U, N, V);
distances_after = zeros(U, N, V);

for n = 1:N
    for u = 1:U
        for v = 1:V
            distances_before(u, n, v) = sqrt((q_x_init(u) - x_v_init(1, v, n))^2 + (q_y_init(u) - 0)^2 + H^2);
            distances_after(u, n, v) = sqrt((sol.q_x(u, n) - x_v_init(1, v, n))^2 + (sol.q_y(u, n) - 0)^2 + H^2);
        end
    end
end

% Plot distances between UAV and vehicles before and after optimization
% figure;
% hold on;
% for v = 1:V
%     plot(1:N, squeeze(distances_before(1, :, v)), '-o', 'DisplayName', ['Vehicle ' num2str(v) ' Pre']);
%     plot(1:N, squeeze(distances_after(1, :, v)), '-x', 'DisplayName', ['Vehicle ' num2str(v) ' Post']);
% end
% xlabel('Time Slot(s)');
% ylabel('Distance (m)');
% legend('show');
% title('Distances Between UAV and Vehicles After Optimization');
% hold off;
